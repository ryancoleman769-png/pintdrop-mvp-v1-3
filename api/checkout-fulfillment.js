...
function scheduleDelivery(sessionId, source) {
  void processCheckoutDeliveries(sessionId, { source }).catch((error) => {
    ...
    if (!voucher) {
      ...
      try {
        ...
        if (triggerDelivery && voucher) {
          scheduleDelivery(sessionId, "checkout-fulfillment:create");
        }
      } catch (error) {
      ...
      scheduleDelivery(sessionId, "checkout-fulfillment:resume");
    }
