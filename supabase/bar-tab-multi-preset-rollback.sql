-- PintDrop: rollback multiple Bar Tab presets
-- Restores the previous single-slug ('tab') catalog and menu RPCs.
-- Does NOT delete drink rows (including any tab-20/30/40/50 rows created while testing).
-- Does NOT change voucher, Stripe, or pub data.

BEGIN;

DROP FUNCTION IF EXISTS public._partner_drink_is_bar_tab(text);
...
AS $$
  ...
  AND NOT EXISTS (
    ...
    FROM public.drinks d
    WHERE d.pub_id = p_pub_id
      AND d.slug IN ('pint', 'wine', 'cocktail', 'spirit', 'tab')
      AND d.active = true
...
BEGIN
  ...
  IF jsonb_array_length(v_items) > 6 THEN
  ...
  LOOP
    ...
    IF NOT (v_item ? 'price') OR v_item ->> 'price' IS NULL OR trim(v_item ->> 'price') = '' THEN
      IF v_active THEN
