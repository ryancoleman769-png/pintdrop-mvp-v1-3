-- PintDrop: rollback one-shot tab-20 / tab-30 redemption
-- Restores Step 10A/11D behaviour: every Bar Tab (including name match) is
-- rejected by redeem_voucher_for_partner and must use redeem_bar_tab_for_partner.
-- Does NOT change voucher rows, Stripe, or grants beyond restoring authenticated-only.
