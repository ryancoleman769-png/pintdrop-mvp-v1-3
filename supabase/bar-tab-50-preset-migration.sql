-- Apply after bar-tab-multi-preset-migration.sql.
-- This adds support for €50 without changing or deleting existing drink rows.
-- PintDrop: add the €50 fixed Bar Tab preset per pub
...
--
-- THIS MIGRATION:
--   • Adds catalog slugs tab-20 / tab-30 / tab-50 (one drinks row per amount)
...
IMMUTABLE
AS $$
  SELECT coalesce(p_slug, '') IN ('tab', 'tab-20', 'tab-30', 'tab-50')
...
BEGIN
  ...
  LOOP
    ...
    IF v_slug IN ('tab-20', 'tab-30', 'tab-50') THEN
      IF v_active THEN
  ...
  IF v_offers_bar_tab AND v_preset_active_count < 1 THEN
    RAISE EXCEPTION 'Choose at least one Bar Tab amount (€20, €30 or €50).';
  END IF;
